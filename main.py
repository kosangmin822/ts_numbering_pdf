...
class PdfAnnotator(QtWidgets.QMainWindow):
    ...
    def save_viewport_parameters(self):
        ...
        try:
            ...
            self._set_dirty()
            QtWidgets.QMessageBox.information(self, "완료", f"{len(selected_rows)}개 항목에 3D 뷰포트 파라메터가 저장되었습니다.")

        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "오류", f"뷰포트 파라메터 저장 중 오류가 발생했습니다:\n{e}")
    ...
    def _setup_3d_viewer_color_controls(self):
        ...
        try:
            ...
            self.color_control_panel.setWindowOpacity(0.5)  # 80% 불투명도 = 20% 투명도 (보이면서도 뒤가 비침)
...
거리: {distance:.2f}"""
    ...
    def __init__(self, pdf_path: Optional[str]=None):
        ...
        self.table.setHorizontalHeaderLabels(["No","Demension Type","Demension","Maximum","Minimum","3d 파라메터"])
    ...
    def open_project(self, path):
        ...
        for m in meta.get("items",[]):
            ...
            it=MarkItem(no=float(m["no"]),page_index=int(m["page_index"]),pdf_point=tuple(m["pdf_point"]),
                        ...
                        viewport_parameters=m.get("viewport_parameters",""))  # 3D 뷰포트 파라메터 복원
